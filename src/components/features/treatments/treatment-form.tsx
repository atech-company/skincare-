...
export function TreatmentForm({
  ...
  useEffect(() => {
    ...
    if (!initial) {
      ...
      if (!state.values.routine_period) {
        state.values.routine_period = "other";
      }

      state.values.status = state.values.status || "scheduled";

    } else if (state.values.session_time) {
      state.values.session_time = state.values.session_time.slice(0, 5);
    }
